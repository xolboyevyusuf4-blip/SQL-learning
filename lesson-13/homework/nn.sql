...
select concat(employee_id,'-',first_name, ' ', last_name) as output from employees
...
select id,
case
when id%2=1 then 'is not boring'
end 
from cinema
where id%2=1
...
select top 1 id, ssn, passportid, itin from person
where id is not null and ssn is not null and passportid is not null and itin is not null
...
10.For every customer that had a delivery to California, provide a result set of the customer orders 
that were delivered to Texas. (Orders Table)
select * from Orders

  select * from Orders
  where DeliveryState = 'CA'
  and CustomerID in (select distinct CustomerID from orders where
  DeliveryState= 'TX')
...
13.The total number of employees in each department and the percentage of those employees who have been 
