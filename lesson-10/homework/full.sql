create database homework10_sql
go
use homework10_sql
...
select name, isnull(departmentname, 0) from Employees
...
select SupplierName, ProductName from Products
...
select Employees.Name, m.ManagerID from Employees
join employees as m
on Employees.EmployeeID=m.ManagerID

7. Using the Students, Courses, and Enrollments tables, write a query to list the names of students who are enrolled in the course named 'Math 101'.
🔁 Expected Columns: StudentName, CourseName

select Students.name, coursename from Students
join Enrollments
...
select departments.DepartmentName, count (EmployeeID) name from Employees
join Departments
on Employees.DepartmentID= Departments.DepartmentID
...
select sales.ProductID, ProductName from Products
left join Sales
on Products.ProductID= sales.ProductID
and sales.ProductID is null
...
select   e1.Name as Employee1, e2.Name as Employee2, e1.ManagerID from Employees as e1
...
select orderid, orderdate, FirstName, lastname from Customers
join orders
on Customers.CustomerID=Orders.CustomerID
...
select e2.Name, e2.Salary from Employees as e1
join Employees as e2
on e1.EmployeeID=e2.EmployeeID

having e2.Salary> (select avg(e1.Salary)
...
select Payments.OrderID, OrderDate from Orders
left join Payments
on Orders.OrderID=Payments.OrderID
...
select * from Employees
select e1.name, e2.name, e1.ManagerID, e2.Salary from Employees as e1
join Employees as e2
on e1.ManagerID=e2.ManagerID
and e1.EmployeeID>e2.EmployeeID
where e2.Salary > 60000

23. Using the Employees and Departments tables, write a query to return employees who work in departments which name 
starts with the letter 'M'.
🔁 Expected Columns: EmployeeName, DepartmentName
select name, departmentname from Employees
join Departments
on Employees.DepartmentID= Departments.DepartmentID
and departmentname like 'm%'
...
select ProductName, saleid, saleamount from products
join sales
on products.ProductID=Sales.ProductID
and saleamount > 500
