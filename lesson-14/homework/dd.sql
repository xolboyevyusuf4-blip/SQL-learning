create database sqlhomework_14
go
use sqlhomework_14
...
select id, left(name, charindex(',', name)-1) as name, right(name, len(name) - charindex(',', name)) 
as surname from TestMultipleColumns
...
select SUBSTRING(vals , 1, charindex('.', vals)-1),
       substring(vals, charindex('.', vals)+1, charindex('.', vals)-1) from Splitter
...
select e1.Id, e1.name, e1.salary from Employee as e1
join Employee as m1
...
select player_id, max(DATEDIFF(day, event_date, GETDATE())) from Activity
group by player_id
 
 3.Your task is to return the third item from that list.(fruits)
 select * from fruits
 select SUBSTRING(fruit_list, CHARINDEX(fruit_list, ',', CHARINDEX(fruit_list, ',', CHARINDEX(fruit_list, ',')+1)+1), len(fruit_list)) from fruits
