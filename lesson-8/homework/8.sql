...
select * from Orders
select min(quantity) as min, max (quantity) as max from Orders
...
select Orders.CustomerID, OrderDate from Orders
left join Invoices
on Orders.CustomerID=Invoices.CustomerID
where Orders.OrderDate between '2023-01-01' and '2023-02-01'
...
on  Products.ProductID=Products_Discounted.ProductID
...
select Products.ProductName from Products 
inner join Products_Discounted
on Products.ProductID = Products_Discounted.ProductID

13.Using Orders table, find the average order amount by year.
select avg (quantity*TotalAmount) from Orders
group by OrderDate
...
select ProductName,
 ...
 from Products
15.Using City_Population table, use Pivot to show values of Year column in seperate columns ([2012], [2013]) and copy results to
a new Population_Each_Year table.
select * from city_population
select district_name, 2012, 2013 from city_population

pivot
for district_name in (2012, 2013) as pivot
