...
select firstname, orderid, orderdate from Orders
join Customers
...
select departmentname, max(Salary) from Employees
join Departments
on Employees.DepartmentID=Departments.DepartmentID
...
select FirstName, orderid, orderdate from Customers
join Orders
...
select * from Orders
select * from Customers
select customers.FirstName, sum(quantity) from Customers
join Orders
on Customers.CustomerID=Orders.CustomerID
group by orders.CustomerID, customers.FirstName
...
select * from Customers
select * from Orders
select FirstName, max(orderdate) as MostRecentOrderDate from Customers
full join Orders
on Customers.CustomerID=Orders.CustomerID
group by FirstName
...
select FirstName, sum(totalamount) from Customers
join Orders
on Customers.CustomerID=Orders.CustomerID
group by FirstName
...
where saledate between '2022-01-01' and '2023-01-01' or sales.SaleAmount> 400
...
join Sales
on Products.ProductID=Sales.ProductID
where Sales.SaleDate like '2023%' and StockQuantity>=100
...
join Departments
on Employees.DepartmentID=Departments.DepartmentID
where HireDate > '2020-01-01' or DepartmentName= 'sales'
🔴 Hard-Level Tasks (7)
14. Return: CustomerName, OrderID, Address, OrderDate
Task: List all orders made by customers in the USA whose address starts with 4 digits.
Tables Used: Customers, Orders

select FirstName, orderid, Address, orderdate from Customers
join Orders
on Customers.CustomerID=Orders.CustomerID
where Address like '____%' and Country= 'usa'
...
select CategoryName, stockquantity as ProductCount  from Categories
join Products
...
select * from Customers
select * from Orders
select firstname, city, orderid, totalamount from Customers
join Orders
on Customers.CustomerID=Orders.CustomerID
...
join Departments
on Employees.DepartmentID=Departments.DepartmentID
where departmentname='Human resources' or departmentname= 'finance' or Name like '%____%'
